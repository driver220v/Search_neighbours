...
@time_dec
def post_pers(user_range):  # reference:  Post person_gen
...
@time_dec
def put_upd(ppl):  # reference:  PUT/ update_coords
    # example:  ppl.items() == {"K": {"coords": [-77,-4]}, "Z": {"coords": [12,14]
    for total in ppl:
        for one in total:
            payload = {'name': f"{one['name']}",
                       'longitude': f'{uniform(10.000, 10.999)}',
                       'latitude': f'{uniform(10.000, 10.999)}'}
...
@time_dec
def delete_one_mhd(ppl):  # reference: DELETE/ delete_pers todo!!!
    for total in ppl:
        for one in total:
            payload = {'name': f"{one['name']}",
                       'longitude': f"{one['longitude']}",
                       'latitude': f"{one['latitude']}"}
...
for i in persons_range:
    ...
    # DELETE METHODS :
    #print(delete_all_mhd())  # 1) DELETE WHOLE TABLE
